...
| `--media-address IP` | none | every peer sends media to this IPv4: the harness rewrites the address of each candidate in Janus's answer (and trickle) before applying it. CI passes the runner's primary host address so S14 reaches the published RTP port through the kernel's DNAT, not the loopback docker-proxy, and runs its preserved branch. Without it peers use Janus's candidates as a viewer would |
...
| S14 | A joins R and brings media up. The harness compares the remote address:port of Janus's selected pair with the endpoints A signalled (its offer's host/srflx candidates; `source_oracle.py`), then ends A's session and reads its record with `docker compose exec janus legion-voice-selfcheck --session <handle> --json` | a rewritten source (the remote address:port is not one of A's endpoints — including A's own gateway address from a proxy's port) reads path `undetermined`; a preserved source reads `direct`. Either branch passes when the record agrees. The info line says which branch ran and why (`own-endpoint`, `own-address-foreign-port`, `foreign-address`): Docker Desktop, and Linux via 127.0.0.1 (docker-proxy), run the rewritten one; Linux with `--media-address <host IP>` the preserved one | the diagnostics never overclaim or underclaim relay/direct against measured ground truth (**A.6**, SC-126) |
...
| S22 | **`JS_JOIN_CAP_REQUIRED=0` only**, and needs `--join-cap-secret`. A joins a declared room with a valid capability; B with one bound to another agent; C with none; D joins an undeclared room with none | all four join. `seen` and `accepted` climb; `refused.cap_wrong_agent` and `refused.cap_missing` each climb by one; `enforced_refusals` stays 0; the undeclared room counts no missing capability | **0.4 shadow**: every capability is verified and counted, and no join is refused (§11.4). Also **O-91**: each of the four sessions carries the per-session marker `join_cap_present` / `join_cap_verdict` (`true`/`ok`, `true`/`cap_wrong_agent`, `false`/`cap_missing`, `false`/`cap_missing`) — the process-wide counters cannot attribute a capability to a session, which is what blocked the 0.6 soak |
...
| S34 | **`JS_JOIN_CAP_REQUIRED=1` only**, and needs `--join-cap-secret`. In a declared room, the REAL connector join code (`connectors/common/peer.py` + `joincap.py`, through `TestPeer`) against a loopback stub of the sim's `/voice/connector/<name>/join-cap` that mints with `_mint_cap`: a peer with no `CONNECTOR_CAP_*`; a configured peer; a second configured peer handed the same capability; a configured peer while the stub answers 404 | bare: `496 cap_missing` and the stub never called; configured: joined, one fetch, session `join_cap_verdict` ok; replay: `496 cap_replayed`; 404: no Janus session created at all for 3 s while the stub counts 4+ retries | **O-88 / slice 0.7b**: a connector is gated by the capability like any join, fetches one per join, and never joins bare. Mutation proof: a peer that joins bare on a failed fetch fails on `S34: with the capability endpoint answering 404 the peer sends no join at all, never a bare one` |
| S35 | **Fail-closed on only.** Declared room, SRC talking throughout, the background heartbeater paused around each constructed heartbeat. Leg A: L armed and hearing; a replace for L at 2; then a heartbeat built at 1 (`as_of` 1) naming L at 1. Leg B: L2 armed by a replace at 3; then a heartbeat built at 2 that omits L2 | A: the late heartbeat does not list L in `stale_listeners`, and L stays at `vis_row` 4 and audible for 2 s; B: L2 reaches row 4, hears SRC, and keeps hearing for 2 s | **O-95 / slice 0.7d**: a heartbeat that predates a batch neither stales nor disarms what that batch applied (design §1.3 amendment, `as_of`). Fails on a pre-0.7d mixer on `S35 leg A: a heartbeat built before L's replace does not list L in stale_listeners`, and with leg A removed on `S35 leg B: a heartbeat built before L2's arming leaves L2 armed, and L2 hears SRC` |
