...
| O-90 | **The `Docs/voice` cross-repo mirror has drifted again, in four files.** O-28 closed 2026-08-31 on a verified full mirror (blob hash both sides). Measured again 2026-09-15 during slice 0.4: `connector-build-plan.md` (sim `d347102272` 09-13 vs mixer `e4ffe1f` 09-01, 25 differing lines), `parcel-voice-semantics.md` (sim `7191e9b6a1` 09-12 vs mixer `6e1415e` 08-31, 10 lines), `per-room-visibility-emission-design-brief.md` (**newer in the mixer**: mixer `1859a7f` 09-13 vs sim `18640868dc` 08-27, 24 lines) and `voice-config-register.md` (**absent from the mixer entirely**). Slice 0.4's own documents are NOT affected: `nonspatial-phase0-design.md` and `voice-programme-ledger.md` hash **SAME** on both sides, so the drift predates this slice and was not introduced by it. | **DEFERRED, not fixed in slice 0.4.** Reconciling needs a per-file decision about which side wins, on content this slice has not reviewed, and one of the four is **ahead in the mixer** — so a blind sim->mixer copy would destroy mixer-side edits. That is a scope boundary below a settled position, so it is deferred with a gate rather than written up as a decision. **Gate:** the next slice that edits any of these four files reconciles that file first, and re-verifies the whole mirror by blob hash on both sides before it closes. | this row; O-28 |
