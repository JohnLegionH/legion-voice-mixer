...
| O-55 | Compose publishes admin (live 24225) and WS (8188) on 0.0.0.0; WS unused by the sim (W-8) | **FIXED IN CODE in the mixer slice-6m commit (`fix(mixer): O-55 admin/WS binds, O-65 fail-closed secrets…`), deployed 2026-09-13 18:28 CDT** (container start 23:28:58Z, image `17be0b8f`): compose publishes admin as `${JS_ADMIN_BIND:-127.0.0.1}:${JS_ADMIN_PORT}:${JS_ADMIN_PORT}`; the base compose no longer publishes WS (opt-in overlay `docker-compose.ws.yml`), and the entrypoint loads the WS transport only when `JS_WS_ENABLED=true` (default `false`: `ws = false` + `transports: { disable = "libjanus_websockets.so" }`). HTTP signalling stays on all interfaces (sim + connectors; `api_secret`-guarded; O-46). Live after deploy: `docker port` shows 24225 → `127.0.0.1` only and no 8188; Janus logged `Transport plugin 'libjanus_websockets.so' has been disabled`; S1+S7 passed. **Operator action open: the live `.env` has no `JS_ADMIN_BIND`, so the sim's `JanusGatewayAdminURI` `192.168.1.225:24225` does not connect (visibility/moderation batches refused) until John adds `JS_ADMIN_BIND=192.168.1.225` and re-runs `docker compose up -d janus`** | audit §2 (W-8) |
