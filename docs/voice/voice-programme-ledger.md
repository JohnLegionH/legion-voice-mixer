...
| O-96 | **A join capability carries the generation the sim ALLOCATED, not the one the mixer APPLIED, so the first join into a room is refused `cap_stale_generation`.** The sim publishes a room's arming state for minting inside `VisAuthority.NextGeneration` - `JoinCapabilityAuthority.Publish(room, EpochString, next)` (`VisAuthority.cs:133-139`) - which runs when a batch is STAMPED, before it is sent. `JanusRoom.cs:80` mints from that published value. The mixer checks the capability against the highest generation it has **applied** in the adopted epoch (`joincap.c:229-238` against `room->vis_policy_gen`). Any generation the sim allocated but the mixer never applied - every batch to a room that does not exist yet is exactly that - makes the next minted capability claim a generation ahead of the mixer. This is the same allocated-versus-applied mistake 0.7d fixed for heartbeats with `as_of` (**O-95**), still present in the capability path | **OPEN (filed 2026-09-17, slice 0.8 soak). Not fixed in that slice.** *Measured live, knob off:* John's four joins gave `join_cap.seen` 5 (with the injector), `accepted` 3, `cap_stale_generation` **2** - `join capability cap_stale_generation for 4fbdfd2a-... in room 226001844` and `... in room 1578726032`, each the first join into that room. The two that passed show why: Elm's per-parcel room 1966197062 had never been addressed by the sim, so nothing was published for it and the capability carried the zero epoch, which a fresh room accepts; Ebony's second join passed because by then the mixer had applied generations 8931-8933. Ebony's published generation had been inflated to ~8,930 by the connector re-arm loop against a room that did not exist (**O-92**, **O-93**), and Transylvania shows the pure form with a single dropped batch: published 1 against applied 0. **Consequence: with `JS_JOIN_CAP_REQUIRED=1` both of those joins would have been REFUSED** and the viewer would have got no voice at the join edge, so this blocks that knob independently of O-88. *Likely fix, not designed here:* publish (epoch, generation) on an APPLIED batch outcome, the same place 0.7d updates `as_of` (`VisAuthority.OnBatchOutcome`), rather than at allocation; or have the mixer accept a capability whose generation is ahead of its applied one, which is weaker because it stops the capability dying with its authority. *Closing requires:* the design decision, the fix, a unit test on the sim side, and a soak with every join accepted | 0.8 soak; `VisAuthority.cs:133-139`, `JanusRoom.cs:80`, `joincap.c:229-238` |
