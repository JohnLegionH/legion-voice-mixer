...
| O-121 | **A single Phlox script timeslice can block the whole region process for over two minutes, and it is the root starver behind O-120.** MEASURED on the live regionserver's own log, 2026-09-20, 06:00-13:38: 26 `WARN [PhloxExe]: Slow timeslice` reports, **p50 9693 ms, max 138064 ms** - one script holding the process for **2 m 18 s**. O-120's investigation ruled out a feeder-local block (its sends are fire-and-forget) and showed the starvation is process-wide: the Watchdog's OWN thread missed its 2500 ms interval by 5406 ms, all three regions' feeders alarmed together, and unrelated `Incoming Packets` threads alarmed with them. In the same window the visibility feeder stalled **635 times**, 33% of those past the mixer's 8000 ms staleness window. V-1b makes voice SURVIVE that (the heartbeat now reports its feed's age and the mixer judges it, O-120), but nothing in V-1b makes the process stop stalling - a script engine that can hold a timeslice for 138 s will starve physics, the packet handlers and anything else on the same box, and voice was simply the subsystem with a watchdog sensitive enough to notice. The home-hosted recipe - sim, mixer and DB on one box - is where it bites first. | **OPEN, out of scope for the voice programme, needs its own slice.** Not a voice defect and not fixable from the voice side. Needs: the scripts behind the worst timeslices identified (the reports name the item UUID - `19520c10-a3d7-4a6a-9fef-148e2643d5a2` is in John's 13:36 log); whether the timeslice budget is being enforced at all or only reported; whether a runaway script can be pre-empted rather than waited out; and a decision on what a sim should do to a script that will not yield. Estimate **investigation 120-180 min** before any fix can be scoped, since the enforcement path has not been read. | slice V-1b; O-120; live regionserver log 2026-09-20 |
